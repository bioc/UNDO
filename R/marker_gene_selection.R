...
function(X,lowper,highper,epsilon1,epsilon2) {
  ...
  Xfilter <- X[sigOrder$ix[(lowper*data_size):(data_size-highper*data_size)],]
  Xfnorm <- sigNorm[sigOrder$ix[(lowper*data_size):(data_size-highper*data_size)]]
  
  k <- Xfilter[,1]/Xfilter[,2]
  eps1 <- epsilon1
  eps2 <- epsilon2
  
  MG1 <- which((k>=(max(k)-eps1*max(k))&k<=max(k)))
  MG2 <- which((k<=(min(k)+eps2*min(k))&k>=min(k)))
